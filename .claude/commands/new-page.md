...
(e.g. `settings` creates `frontend/app/settings/page.tsx`, `settings/profile` creates `frontend/app/settings/profile/page.tsx`)

## Instructions

1. Parse the route from $ARGUMENTS. Create the directory `frontend/app/<route>/`.
...
3. If the page fetches data, use the `frontend/api/` layer (see `frontend/api/datasources.ts` for the pattern). Wrap async data in a `<Suspense>` boundary with a fallback.
