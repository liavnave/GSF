/**
 * Не React-компонент: чистые функции, без "use client".
 * Их можно вызывать и на сервере (например, после fetch в Server Component), и в клиенте.
 * Сборка секций для SinglePageComposer по `focus` и дереву `Database[]`.
 */
import type { SinglePageFormat } from '@/components/dataPage/SinglePageView';
import type { ColumnField, Database, Column, Entity, Table } from '@/types/datasources';
...
function inTables(db: Database, schema: Entity, focusId: string): TreeResolved | null {
