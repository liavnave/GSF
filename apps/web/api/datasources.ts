...
export const datasources = {
	getDBs: (filters: DataFilters) =>
		requests.get<ResponseWithCount<Database[]>>('/api/datasources/dbs', filters),
	getTablesByDatabaseId: (dbId: string, filters: DataFilters) =>
		requests.get<ResponseWithCount<Table[]>>(`/api/datasources/dbs/${dbId}/tables`, filters),
	getSchemasByDatabaseId: (dbId: string, filters: DataFilters) =>
		requests.get<ResponseWithCount<Entity[]>>(`/api/datasources/dbs/${dbId}`, filters),
	getAllSchemas: () => requests.get<Array<Entity & { db: NameId }>>('/api/datasources/schemas'),
	getTablesBySchemaId: (schemaId: string, filters: DataFilters) =>
		requests.get<ResponseWithCount<Table[]>>(`/api/datasources/schemas/${schemaId}`, filters),
	...
	getColumnsByDatabaseId: (dbId: string, filters: DataFilters) =>
		requests.get<ResponseWithCount<Column[]>>(`/api/datasources/dbs/${dbId}/columns`, filters),
